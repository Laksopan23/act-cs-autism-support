...
const NewAnalysisDoctor = () => {
    ...
    const [inputType, setInputType] = useState(
        location.state?.prefillText ? 'text' : location.state?.prefillAudio ? 'audio' : 'audio'
    );
    ...
    useEffect(() => {
        ...
        if (prefillAudio && prefillAudio.startsWith('data:')) {
            fetch(prefillAudio)
                ...
                .catch((conversionError) => console.error('Failed to convert audio:', conversionError));
        }
    ...
    useEffect(() => {
        ...
        therapyApi.get(`/api/doctor/patients/${selectedPatientId}/analyses`)
            .then((res) => {
                if (!cancelled) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (inputType === 'audio') {
                ...
                if (!textInput.trim()) {
                    throw new Error('Please enter text describing the symptoms.');
                }

                formData.append('transcript', textInput);
            }

            const response = await therapyApi.post(`/api/doctor/patients/${selectedPatientId}/analyses`, formData);
            const savedAnalysis = response.data.analysis;

            setLatestAnalysis(savedAnalysis);
            setAnalysisHistory((previous) => [savedAnalysis, ...previous.filter((item) => item._id !== savedAnalysis._id)]);
            setSuccess('Analysis processed, treatment suggestions generated, and the record was saved.');
        } catch (submitError) {
            setError(submitError.response?.data?.message || submitError.message || 'Analysis failed. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleReviewSaved = (updatedAnalysis) => {
        ...
        setAnalysisHistory((previous) => previous.map((item) => (
            ...
            <div className="flex-1 overflow-auto">
                <div className="max-w-6xl mx-auto p-8 page-enter">
                    <div className="mb-10 text-center relative z-10">
                        ...
                        <p className="text-slate-400 font-medium">Capture voice notes or text, then review the AI results and saved history below.</p>
                    </div>
                        ...
                        <form onSubmit={handleSubmit} className="relative z-10">
                            <div className="flex bg-white/[0.03] border border-white/[0.06] p-1.5 rounded-2xl mb-8 w-fit mx-auto shadow-inner">
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-8 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${inputType === 'audio' ? 'bg-gradient-to-r from-violet-500/30 to-violet-500/10 text-violet-300 shadow-sm border border-violet-500/30' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.05]'}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-8 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${inputType === 'text' ? 'bg-white/[0.1] text-slate-200 shadow-sm border border-white/[0.1]' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.05]'}`}
                                >
                            ...
                            {inputType === 'text' && (
                                <div className="mb-8 relative">
                                    <label className="text-[10px] font-bold text-violet-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                        <FiFileText /> Clinical Notes and Behavioral Observations
                                    </label>
                                    <textarea
                    ...
                    {latestAnalysis && (
                        <section ref={resultsRef} className="mt-8 card p-8 border-subtle card-glow">
                            <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                ...
                                <button
                                    onClick={() => navigate(`${BASE}/doctor/patients/${selectedPatientId}?tab=analyses`)}
