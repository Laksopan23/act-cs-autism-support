...
const SEVERITY_CLASSES = {
    mild: 'badge-low border-none',
    moderate: 'badge-medium border-none',
    severe: 'badge-high border-none'
};
...
const PatientProfile = () => {
    ...
    const fetchPatientData = async () => {
        try {
            const [patientRes, analysesRes] = await Promise.all([
                therapyApi.get(`/api/doctor/patients/${id}`),
                therapyApi.get(`/api/doctor/patients/${id}/analyses`)
            ]);
    ...
    const handleAnalysisUpdated = (updatedAnalysis) => {
        setAnalyses((previous) => previous.map((item) => (
            item._id === updatedAnalysis._id ? updatedAnalysis : item
        )));
    };

    const handleDownloadReport = async (analysisId) => {
        try {
            await downloadAnalysisReport('doctor', analysisId);
        } catch (downloadError) {
        ...
        return (
            <div className="flex min-h-screen bg-app">
                ...
                <div className="flex-1 flex items-center justify-center flex-col text-slate-500">
                    <FiUser size={32} className="mb-4 opacity-50" />
                    <Link to={`${BASE}/doctor/dashboard`} className="text-violet-400 hover:text-violet-300">
                        Patient not found. Back to Dashboard
                    </Link>
                </div>
            </div>
        );
                ...
                <div className="max-w-7xl mx-auto p-8 page-enter">
                    <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-6">
                            <Link to={`${BASE}/doctor/patients`} className="w-10 h-10 rounded-full bg-white/[0.05] border border-white/[0.1] flex items-center justify-center text-slate-400 hover:text-violet-400 hover:bg-violet-500/10 transition-all card-glow">
                                <FiArrowLeft />
                            </Link>
                            <div className="flex items-center gap-5">
                                <div className="w-16 h-16 gradient-primary rounded-2xl flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-violet-500/20">
                                    {patient.name[0]}
                                </div>
                                <div>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className={`btn-secondary flex items-center gap-2 ${activeTab === 'chat' ? 'bg-violet-500/20 text-violet-300 border-violet-500/40 shadow-lg shadow-violet-500/10' : ''}`}
                            >
                                <FiMessageSquare /> <span className="hidden sm:inline">Consultation</span>
                            </button>
                            <button onClick={() => navigate(`${BASE}/doctor/new-analysis/${id}`)} className="btn-primary flex items-center gap-2">
                                <FiPlus /> New Analysis
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 fade-in">
                            <div className="lg:col-span-2 space-y-8">
                                <section className="card p-8 card-glow relative overflow-hidden">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-8 relative z-10">
                                        ...
                                        <InfoBlock label="Diagnosis Severity" value={patient.diagnosisDetails?.severity || 'N/A'} isPill pillClass={SEVERITY_CLASSES[patient.diagnosisDetails?.severity] || 'badge-neutral'} />
                                ...
                                <section>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {analyses.length > 3 && (
                                            <button onClick={() => setActiveTab('analyses')} className="w-full py-4 text-xs font-bold text-slate-500 uppercase tracking-widest hover:text-violet-400 transition-colors">
                                                View All {analyses.length} Records -&gt;
                                            </button>
                                        )}
                                    </div>
                                </section>
                                ...
                                <section className="card p-8 card-glow relative overflow-hidden">
                                    <div className="absolute top-0 right-0 p-8 opacity-[0.03]"><FiShield className="w-32 h-32" /></div>
                                    <h3 className="text-xl font-bold text-slate-200 mb-6 flex items-center gap-2 relative z-10"><FiShield className="text-cyan-400" /> Current Care Plan</h3>
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3 relative z-10">
                                        ...
                                        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                                            <p className="text-[10px] font-bold uppercase tracking-widest text-amber-300">Awaiting Review</p>
                                            <p className="mt-2 text-3xl font-black text-slate-100">{analyses.length - reviewedAnalyses.length}</p>
                                        </div>
                                    </div>
                                </section>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="card p-8 h-fit">
                                    ...
                                    <UrgencyDonutChart data={chartData} emptyMessage="Insufficient Data" />
                                </div>
                        ...
                        <div className="space-y-4 fade-in">
                            ...
                            {analyses.length === 0 ? (
                                ...
                                analyses.map((analysis) => (
                                    <div key={analysis._id} className="space-y-4">
                                        <AnalysisCard analysis={analysis} isDoctorView />
                                        <DoctorReviewPanel analysis={analysis} onSaved={handleAnalysisUpdated} />
                                    </div>
                                ))
                            )}
                        </div>
                    ...
                    {activeTab === 'care-plan' && (
                        <div className="fade-in">
                            <CarePlanBoard
                                ...
                                onOpenReview={() => setActiveTab('analyses')}
                            />
                        </div>
                    )}
