...
import { FiActivity, FiChevronDown, FiClipboard, FiMessageSquare, FiShield, FiTarget, FiVolume2 } from 'react-icons/fi';
import {
    formatAnalysisLabel,
    getCareStageLabel,
    getDisplayedSummary,
...
const AnalysisCard = ({ analysis, isDoctorView = false }) => {
    ...
    const getUrgencyConfig = (urgency) => {
        switch (urgency) {
            case 'high':
                return { bg: 'bg-rose-500/10', text: 'text-rose-400', border: 'border-rose-500/20', line: 'bg-rose-500' };
            case 'medium':
                return { bg: 'bg-amber-500/10', text: 'text-amber-400', border: 'border-amber-500/20', line: 'bg-amber-500' };
            case 'low':
                return { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', line: 'bg-emerald-500' };
            default:
                return { bg: 'bg-violet-500/10', text: 'text-violet-400', border: 'border-violet-500/20', line: 'bg-violet-500' };
        }
    };
            ...
            <div className="p-6 sm:p-8 pl-10">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/[0.05] pt-5">
                    <div className="flex flex-wrap gap-2">
                        {analysis.issueTop3?.slice(0, 2).map((issue, index) => (
                            <span key={index} className="px-3 py-1 bg-white/[0.03] text-slate-400 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-white/[0.05]">
                            ...
                            <div className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        {analysis.issueTop3?.map((issue, index) => (
