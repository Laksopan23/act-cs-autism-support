...
const ChildProfile = () => {
    ...
    if (!child) {
        return <div className="flex min-h-screen bg-app"><Sidebar /><div className="flex-1 flex items-center justify-center text-slate-500">Child not found</div></div>;
    }
    ...
    const reviewedAnalyses = analyses.filter(a => a.doctorReview?.reviewedAt);
    const ongoingTreatments = reviewedAnalyses.filter(a => a.doctorReview?.treatmentStatus !== 'completed' && a.doctorReview?.careStage !== 'completed');
            ...
            <div className="flex-1 overflow-auto">
                <div className="max-w-7xl mx-auto p-8 page-enter">
                    <div className="mb-8">
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                            ...
                            <div className="flex flex-wrap gap-3">
                                <button onClick={() => navigate(`/parent/children/${id}?tab=care-plan`)} className="btn-primary flex items-center gap-2">
                                    <FiShield /> Care Plan
                                </button>
                    ...
                    <div className="flex p-1.5 bg-white/[0.03] border border-white/[0.06] rounded-xl w-fit mb-8 shadow-inner">
                        {tabs.map(t => (
                            <button key={t.id} onClick={() => setActiveTab(t.id)} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === t.id ? 'bg-white/[0.08] text-violet-300 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>{t.label}</button>
                        ))}
                    </div>
                    ...
                    {activeTab === 'analyses' && (
                        <div className="space-y-4 fade-in">
                            ...
                            {analyses.length === 0 ? <p className="text-slate-500">No analysis records found.</p> : analyses.map(a => <AnalysisCard key={a._id} analysis={a} />)}
                        </div>
                    )}
