...
async function ensureChild({ parent, doctor, childData }) {
    ...
    if (!child) {
        ...
        child.assignedDoctors = [
            ...new Set([...(child.assignedDoctors || []).map((id) => id.toString()), doctor._id.toString()])
        ];
        await child.save();
    }
    return child;
...
async function ensureAnalysis({ child, performedBy, issueLabel, urgencyLabel, transcript, summary, reviewed, doctor }) {
    ...
    const urgencyTop3 = pickTop3(
        urgencyLabel,
        urgencyLabel === 'high' ? 'medium' : 'high',
        urgencyLabel === 'low' ? 'medium' : 'low'
    );
